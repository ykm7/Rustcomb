...
pub fn create_files(
    ...
    // Read content once and reuse for all files
    let content = std::fs::read(&filename_path)
        .unwrap_or_else(|_| panic!("Failed to read {}", filename_path.display()));

    use rayon::prelude::*;
    (1..=num_of_files_to_create)
        .into_par_iter()
        .for_each(|idx| {
